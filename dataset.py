...
from data_utils import rand_train_test_idx, even_quantile_labels, dataset_drive_url
...
def load_deezer_dataset():
    ...
    A, label, features = deezer['A'], deezer['label'], deezer['statistics']
    edge_index = torch.tensor(A.nonzero(), dtype=torch.long)
...
def load_ogb_dataset(name):
    ...
    dataset.get_idx_split = ogb_idx_to_tensor
...
def load_twitch_gamer_dataset(task="mature", normalize=True):
    print(f'{DATAPATH}twitch-gamer_edges.csv')
...
def load_wiki():
    print(dataset_drive_url['wiki_features'])
    print(dataset_drive_url['wiki_edges'])
    print(dataset_drive_url['wiki_views'])
    ...
    print(f"statistics shape: {features.shape[0]}")
